...
`source_host` - более общее поле, заполняемое механизмом логирования, указывающее источник события лога или имя хоста, где лог был создан.
`server_name` (sn) - специально привязано к `server_name` в конфигурации NGINX, помогая определить, какой блок виртуального сервера обработал запрос.
